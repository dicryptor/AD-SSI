...
function Controller() {
    ...
    var containers = [];
    for (x = 0; x < 5; x++) containers[x] = Ti.UI.createView({
        height: Ti.UI.SIZE,
        bottom: Alloy.Globals.calculateHeight(15),
    ...
    submitbtn.addEventListener("click", function() {
        ...
        if ("" == devid.value || null == devid.value) {
            dialog.message = "Device ID is required. Please try again!";
            dialog.show();
            return false;
        }
        if (false == Alloy.Globals.CheckInternetConnection) {
    ...
    $.register.addEventListener("swipe", function(e) {
        "right" == e.direction && $.register.close();
    });
    setTimeout(function() {
